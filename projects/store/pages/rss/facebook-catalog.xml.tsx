import { RssFacebookCatalog } from '@lab77store/domain';
...
export const getServerSideProps = async ({ res }) => {
	...
	const response = await axios.get<RssFacebookCatalog>(`/rss/facebook-catalog`);
	const { products } = response.data;
	const items = products
		.map(
			product => `
	            <item>
	                <g:id>${product.variationId}</g:id>
					<g:item_group_id>${product.productId}</g:item_group_id>
	                ...
	                <g:price>${product.oldPrice || product.price} BRL</g:price>
					${product.oldPrice ? `<g:sale_price>${product.price} BRL</g:sale_price>` : ''}
	                <g:brand>Lab77</g:brand>
					<g:additional_image_link>${product.photos.join(',')}</g:additional_image_link>
	...
	const sitemap = `<?xml version="1.0" encoding="UTF-8"?>
					<rss xmlns:g="http://base.google.com/ns/1.0" version="2.0">
                        <channel>
                            <title>Lab77</title>
                            <link>${baseUrl}/rss/facebook-catalog.xml</link>
